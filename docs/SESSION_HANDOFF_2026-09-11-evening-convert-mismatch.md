...
1. **编辑脚本列名对齐用例**（用户 9-12 补充澄清）：列头格式改为"元素（目标）/ 值（测试数据）/ 期望值（预期结果）"——括号里写用例**列名**（目标/测试数据/预期结果），不写"用例"字样。用户核心观点：**用例的每一列都能与编辑脚本列一一对应，所以转换后编辑脚本的每一列都必须有值**（目标→元素、测试数据→值、预期结果→期望值），不允许出现空白列。列头改动 + RC1 修复后保证每列有值。
2. **识别验证码拆两步**（用户 9-12 补充：**AI 用例生成也要拆**）：
   - **源头**：AI 用例生成（#2 模块）产出用例时就写两步——步骤A"识别验证码，目标=验证码图片"；步骤B"输入验证码，目标=验证码输入框，数据=识别结果"。用例层不再有合并的一步。
   - **转脚本**：用例转自动化按用例的两步转两步（识别验证码→步骤A；输入→步骤B），不再需要 input_captcha 合并语义。
   - 涉及：AI 用例生成的提示词/规则（写两步）、STEP1_PROMPT（删"无论原文一步还是两步都合并"的规则，改为按用例原样两步）、step_codegen/executor 的 input_captcha 分支保留兼容旧数据但新链路产出两步。
...
4. 用户新需求 1-3（列名标注【目标/测试数据/预期结果】+每列必有值/验证码拆两步/选择器重设计）
