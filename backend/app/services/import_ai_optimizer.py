...
- action（动作）：单个动词，只能是：点击/输入/选择/勾选/打开/进入/等待/断言/识别验证码。描述句里无论写成"账号输入test02"还是"输入账号"，动词都归一到"输入"。
...
凡步骤描述涉及验证码（识别图形验证码/输入验证码/填验证码等），无论原文写成一步还是两步，【必须合并拆成一步】，且：
- action 固定为「识别验证码」（这是平台标准动作词，执行引擎靠它走 OCR 流程）
- target = 验证码图片元素（如「图形验证码图片」）
- data = 验证码输入框（识别结果要填入的输入框）
- expected = 「验证码输入框自动填入识别出的验证码」
- 拆分后，后续步骤不得再出现单独的「输入验证码」步骤

输入描述："4.识别图形验证码，输入验证码"
输出步骤：{"step":4, "action":"识别验证码", "target":"图形验证码图片", "data":"验证码输入框", "expected":"验证码输入框自动填入识别出的验证码"}

输入描述："6.在验证码输入框输入图片中的验证码"
输出步骤：{"step":6, "action":"识别验证码", "target":"图形验证码图片", "data":"验证码输入框", "expected":"验证码输入框自动填入识别出的验证码"}
